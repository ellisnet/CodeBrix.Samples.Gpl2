...
public class ModulationEnvelopeTinySoundFontTests
{
    ...
    {
        var dir = Path.Combine(TestSettings.ReferenceDataDirectory, "TinySoundFont", "ModulationEnvelope", "D03_A05_H07_D11_S02_R13");
    ...
    {
        var dir = Path.Combine(TestSettings.ReferenceDataDirectory, "TinySoundFont", "ModulationEnvelope", "D00_A00_H00_D23_S01_R57");
        var expected = File.ReadLines(Path.Combine(dir, "Continuous.csv")).Select(line => float.Parse(line.Split(',')[1], CultureInfo.InvariantCulture)).ToArray();
        expected.Length.Should().Be(3000);
