...
namespace ManagedDoom
{
    ...
    public static class SaveAndLoad
    {
        ...
        // Serializes the current game to the raw vanilla save-game bytes. This
        // is the seam the storage layer persists (verbatim to a .dsg file by
        // default, base64 in settings.sqlite in the app).
        public static byte[] Save(DoomGame game, string description)
        {
            var sg = new SaveGame(description);
            return sg.Save(game);
        }

        // Writes a save-game file (a thin wrapper over the byte-producing Save,
        // kept for the default file-based storage).
        public static void Save(DoomGame game, string description, string path)
        {
            var data = Save(game, description);
        ...
        // Restores a game from the raw vanilla save-game bytes.
        public static void Load(DoomGame game, byte[] data)
        {
        ...
        // Reads a save-game file (a thin wrapper over the byte-consuming Load,
        // kept for the default file-based storage).
        public static void Load(DoomGame game, string path)
        {
            Load(game, File.ReadAllBytes(path));
        }
        ...
        private class SaveGame
        {
            ...
            public byte[] Save(DoomGame game)
            {
